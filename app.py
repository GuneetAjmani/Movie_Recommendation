...
if not st.session_state.username:
    ...
    # Description
    st.markdown('<div class="subheader">Welcome to the Movie Recommender System! Select a movie and get recommendations based on your favorite movies.</div>', unsafe_allow_html=True)
    ...
    if st.button('Show Recommendation'):
        st.session_state.num_recommendations = 5
        st.session_state.recommended_movie_details = recommend(selected_movie, st.session_state.num_recommendations)

    if 'recommended_movie_details' in st.session_state and st.session_state.recommended_movie_details:
        recommended_movie_details = st.session_state.recommended_movie_details

        st.subheader('Top Recommended Movies')
        cols = st.columns(5)
        for idx, (name, poster, url, rating) in enumerate(recommended_movie_details):
            col = cols[idx % 5]
            with col:
                st.image(poster)
                st.write(f"**{name}**")
